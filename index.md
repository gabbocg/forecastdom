...
**forecastdom** is a toolkit for (un)conditional forecast dominance
testing in R. It covers the full taxonomy of forecast evaluation
hypotheses (unconditional and conditional, equal and superior) plus
encompassing, nested-model, predictive regression, and parameter
instability tests.
...
The package covers the forecast evaluation taxonomy from Li, Liao, and
Quaedvlieg (2022):
...
The CSPA test uses Rcpp-accelerated C++ code for the computationally
intensive operations (Gaussian process column-max and binary search
p-value computation).
